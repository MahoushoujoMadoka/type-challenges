...
> = Str extends `${infer Item}&${infer Rest}` ? Item extends `${infer Key}=${infer Val}`?ParseQueryString<Rest,{
  [k in keyof Result|Key]:K extends keyof Result? Result[K]:Val extends string?Val:true:never
}>  : Result
