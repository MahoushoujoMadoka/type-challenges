...
> = Str extends `${infer Item}&${infer Rest}`
  ...
  : [...StrArr,Str]
  type Normalize<T extends string,Result extends string[]=[] >=T extends [infer Item,...infer Rest]? Item extends `${infer _}=${infer _}`?Normalize<Rest,[...Result,Item]>:Normalize<Rest,[...Result,Item]>:1
