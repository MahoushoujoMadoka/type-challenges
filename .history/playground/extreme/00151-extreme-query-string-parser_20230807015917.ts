...
type Include<T extends unknown[], Search> = T extends [infer Item, ...infer Rest]
  ? Search extends Item
    ...
    : Include<Rest, Search>
  : false
type a1 = Collect<'k1=v1&k1=v2&k1=v1'>
type a2 = Normalize<a1>
type a3 = ProcessArr<a2>
    ...
    ? ProcessArr<
        Rest,
        {
          [K in keyof Result | Key]: K extends Key
            ? Result[Key] extends string | true | 'true'
              ? Result[Key] extends ParseTrueStr<Val>
                ? ParseTrueStr<Val>
                : [Result[Key], ParseTrueStr<Val>]
              : Result[Key] extends unknown[]
              ?Include<Result[Key],ParseTrueStr<Val>>extends true [...Result[Key], ParseTrueStr<Val>]
              : ParseTrueStr<Val>
            : Result[K]
        }
      >
