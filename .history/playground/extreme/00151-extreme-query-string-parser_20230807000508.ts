...
> = Str extends `${infer Item extends `${infer Key}=${infer Val}}&${infer Rest}` ? Item `?ParseQueryString<Rest,{
  [K in keyof Result|Key]:K extends keyof Result? Result[K]:Val   extends string?Val:true
}>  : Result:
