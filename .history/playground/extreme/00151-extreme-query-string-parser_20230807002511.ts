...
> = Str extends `${infer Key}=${infer Rest}`
  ? Rest extends `${infer Val}&${infer Rest}`
    ? ParseQueryString<
        Rest,
        {
          [K in keyof Result | Key]: K extends keyof Result
    ...
    : ParseQueryString<
        Rest,
        {
          [K in keyof Result | Rest]: K extends keyof Result ? Result[K] : true
        }
      >
  : ParseQueryString<
      Rest,
      {
        [K in keyof Result | Str]: K extends keyof Result ? Result[K] : true
      }
    >
